...
Three independent ways the overlay can get stuck at `0:00`:
...
3. **Zero-countdown cleanup depended on a second runtime round trip.**
   After a later closed-lid/locked-session report, the helper had clearly
   received `updateBreak { remainingMs: 0 }` but did not clear for at least 30s
   after unlock. This means the Swift zero watchdog was not independent enough
   to be the only recovery layer. The macOS backend now finishes the helper
   directly when its break timer reaches zero, before notifying the runtime.
...
- The Rust parent/backend owns the zero-countdown finish path. Swift watchdogs
  remain as secondary helper-local defense, not the primary guarantee.
...
- **`src/macos_helper.rs` (`MacOSHelperCore`)** — macOS helper session/timer
  state was split out from the process-owning backend so the overlay finish path
  is unit-testable. When an overlay sample reaches zero remaining time, the
  backend now sends `finishBreak` immediately instead of sending
  `updateBreak { remainingMs: 0 }` and waiting for the runtime to send
  `FinishBreak` later. The runtime still receives `BreakFinished` to update
  scheduler state; the later runtime `FinishBreak` is idempotently ignored by the
  backend because the helper has already been cleared.
- **`helpers/macos-helper/.../main.swift`** — two `DispatchSourceTimer`
  ...
  - **Watchdog A (zero-timer, 1s):** armed once on the first `updateBreak` with
    ...
    non-zero update. It now exists only as secondary defense because the backend
    normally finishes directly before sending a zero update.
...
- Stuck at `0:00`, healthy helper, finish never sent → backend-owned zero finish
  avoids the zero update in normal operation; Watchdog A remains a last-resort
  helper-local clear if a zero update is ever observed.
...
- `src/macos_helper.rs::tests::finished_overlay_sample_finishes_helper_without_zero_update`
  — a finishing overlay sample sends `pollActivity` followed directly by
  `finishBreak`; the later runtime `FinishBreak` command is ignored.
- `src/macos_helper.rs::tests::finished_overlay_sample_preserves_lock_request`
  — a lock-after request discovered in the finishing sample is preserved in the
  direct `finishBreak` command and still queues the sync/runtime lock event
  before `BreakFinished`.
...
- `nix develop --command cargo test --lib macos_helper` passes (27 tests).
- `make check` passes (fmt-check, clippy `-D warnings`, 277 tests).
- `make macos-helper-build` passes (`swift build`).
- Manual macOS verification pending on a machine with Accessibility/Input
  ...
  - Closed-lid/locked break: start a short break, close/lock until it expires,
    unlock, and confirm the overlay clears immediately through backend-owned
    finish instead of sitting at `0:00`.
  - Stuck-at-zero fallback: with `RUSTEYES_HELPER_WATCHDOG_MS` short, force a
    zero update and confirm Watchdog A clears the overlay and restores input.
